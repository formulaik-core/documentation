...
We are going to build step by step the Formulaik generator app.
...
```bash
npx formulaik app new 
```
The formulaik will ask a series of questions:
...
### Add the command
```app new``` will generate a new Formulaik app project.
...
export default ({
  _formulaikType: 'command',
  name: 'new',  
  description: `Generate a Formulaik app 🚀`,
  questions: [
  ],
...
```bash
npx formulaik app new -h
```

```bash
Generate a Formulaik app 🚀
...
Examples:
  formulaik app new --appID='MyApp'
```

### Build the handler 

We can now build the command handler. Since we are going to generate a project, we'll need a good generator. We'll use the  [*chunks extension*](https://github.com/@formulaik/chunk-extension) that's made for this task.
#TODO
...
The project is available at [https://github.com/formulaik-org/generator](https://github.com/formulaik-org/generator)
