...
# Use with an external components library

A Formulaik [component library](../concepts/components#component-library) is a set of Formulaik compatible inputs each with a unique key.
...
In this guid we will use an existing component library that adapts some of the Google Material UI components: [formulaik-mui-react](https://github.com/formulaik-community/formulaik-mui-react).

## Declare inputs
An [input](../concepts/terms.mdx#input) is a JSON declaration for formulaik to show a component referenced by a the `component` key.
